...
const HistorySidebar = () => {
    ...
    return (
        <div className="p-4 bg-gray-900 dark:bg-gray-300 h-full text-gray-100 dark:text-gray-900 flex flex-col relative">
            <div className="mb-6">
            ...
            <div className="flex-1 overflow-y-auto space-y-6">
                {history.length === 0 ? (
                    <div className="flex items-center justify-center h-full">
                        <p className="text-gray-300 dark:text-gray-600 text-base">No History Found. Chat to Continue</p>
                    </div>
                ) : (
                    Object.entries(groupedHistory).map(([date, items]) => (
                    <div key={date} className="space-y-1">
                        <h3 className="text-xs text-gray-300 dark:text-gray-600 font-medium mb-2 px-2">{date}</h3>
            ...
            {activeDropdown !== null && (
                <div 
                    ...
                    className="py-1.5 w-36 bg-gray-800 dark:bg-gray-50 rounded-lg shadow-xl ring-1 ring-black ring-opacity-5 z-50 
                             animate-in fade-in duration-200 scale-95 origin-top-right"
                >
                    <button
                        onClick={() => startRename(activeDropdown)}
                        className="block w-full text-left px-4 py-2 text-base text-gray-200 dark:text-gray-800 hover:bg-gray-700 dark:hover:bg-gray-200 transition-colors"
                    >
                        Rename
                    </button>
                    <button
                        onClick={() => handleDeletePrompt(activeDropdown)}
                        className="block w-full text-left px-4 py-2 text-base text-red-400 dark:text-red-600 hover:bg-gray-700 dark:hover:bg-gray-200 transition-colors"
                    >
                        Delete
                    </button>
                </div>
